...
```{iframe} ../sims/ch02-sse-minimum.html
:class: sim sim-m
:width: 100%
Two sliders set your own intercept $b_0$ and slope $b_1$ on the 25 Toluca runs. The readouts compare the $Q(b_0, b_1)$ your line achieves against the smallest value any line can achieve.
```

**What to notice.** No pair of slider settings pushes $Q$ below $54{,}825.5$, and the settings
that come closest read $62.5$ and $3.57$, the least-squares estimates. **Try this.** Hold the
slope at $3.57$ and sweep the intercept alone: the sum of residuals crosses zero at exactly the
setting where $Q$ bottoms out, which is the first normal equation happening in front of you.
Back to [Section 2.2](#ch02-least-squares).
...
The identities in Theorem 2.6 are exact, so the fastest way to believe them is to try to break
one.

```{iframe} ../sims/ch02-residual-identities.html
:class: sim sim-m
:width: 100%
Drag any of the six hand-computation runs anywhere on the plot. The line refits instantly, and the four readouts track $\sum e_i$, $\sum X_i e_i$, the mean of $Y$, and the mean of $\hat{Y}$.
```

**What to notice.** However badly you mangle the data, $\sum e_i$ and $\sum X_i e_i$ stay at
zero and the mean of $\hat{Y}$ stays equal to $\bar{Y}$. **Try this.** Drag one run to the very
top of the plot: the residuals become huge, the fit becomes useless, and the identities do not
move, which is why they check your arithmetic and never your model.
Back to [Section 2.3](#ch02-residuals).
...
Theorem 2.8 says the divisor $n-2$ is the one that is right on average, and repeated sampling
is the way to watch that claim settle.

```{iframe} ../sims/ch02-mse-divisor.html
:class: sim sim-m
:width: 100%
Each sample is 25 fresh runs from a model whose error variance really is $\sigma^2 = 2383.4$. The histogram collects $\mathrm{MSE} = \mathrm{SSE}/(n-2)$, and the readouts show what each divisor gives on average.
```

**What to notice.** Individual values of $\mathrm{MSE}$ scatter widely, most of them landing
anywhere from about $1200$ to about $4000$, yet their average walks steadily toward $2383.4$. **Try this.** Press Draw 1000 twice and
compare the two means: the $n-2$ divisor lands on $\sigma^2$ while the $n$ divisor stays about
$8$ percent low no matter how many samples you add, which is what bias means.
Back to [Section 2.4](#ch02-sigma).
...
The bundle of fitted lines in @fig-ch02-sampling-lines is easier to believe when you build it
one sample at a time.

```{iframe} ../sims/ch02-slope-variance.html
:class: sim sim-l
:width: 100%
Every sample reuses the 25 Toluca lot sizes with fresh errors around the line $62.366 + 3.5702X$. The strip on top is the newest sample of work hours; the histogram below collects the fitted slope $b_1$ from all of them.
```

**What to notice.** Draw samples one at a time and no single $b_1$ equals $3.5702$, yet the pile
they build is centered there. **Try this.** Press Draw 1000 and compare the SD of $b_1$ readout
with $\sigma/\sqrt{S_{xx}} = 0.3469$ sitting beside it: the variance formula of Theorem 2.9 is
not an approximation, it is the width of this pile.
Back to [Section 2.5](#ch02-gauss-markov-slr).
