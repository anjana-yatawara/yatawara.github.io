...
The widget below does the holding fixed for you, one population level at a time, so you can watch the
gentle partial slope and the steep marginal slope stop looking like rivals.

```{iframe} ../sims/ch08-holding-fixed.html
:class: sim sim-m
:width: 100%
...
Before you trust any single row of a sequential table, promote and demote the predictors yourself and
watch $F^{*}$ re-decide whether the newest one earned its place.

```{iframe} ../sims/ch08-extra-ss.html
:class: sim sim-m
:width: 100%
...
Rather than take the recipe on faith, pull the cleaning slider below and watch the marginal slope turn
into the coefficient in front of you.

```{iframe} ../sims/ch08-avplot.html
:class: sim sim-m
:width: 100%
