...
The row-by-column rule is easier to trust once you have watched it happen on the
one product a regression actually needs, so build $\mathbf{X}'\mathbf{X}$ yourself
before moving on.

```{iframe} ../sims/ch06-matrix-product.html
:class: sim sim-m
:width: 100%
...
A determinant of zero is easier to feel than to define, so pull the two columns
together yourself and watch the inverse come apart long before they meet.

```{iframe} ../sims/ch06-singular.html
:class: sim sim-m
:width: 100%
...
The projection is the one idea in this chapter a flat page cannot really show, so
turn the picture around and move $\mathbf{Y}$ with your own finger.

```{iframe} ../sims/ch06-projection.html
:class: sim sim-m
:width: 100%
...
:name: fig-ch06-mvn
:alt: A scatter of about twelve hundred gray points forming a tilted elliptical cloud centered at the origin, with concentric blue elliptical density contours drawn over it. The cloud stretches along the forty-five degree diagonal because the two coordinates have a positive covariance of 0.8.
