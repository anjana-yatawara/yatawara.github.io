...
The fastest way to feel why an elasticity is a percentage is to set one yourself and read
the translation as it changes.

```{iframe} ../sims/ch10-log-elasticity.html
:class: sim sim-m
:width: 100%
Set the intercept $b_0$ and the elasticity $b_1$ for the 62 mammals on the log-log scale, and watch the exact and approximate percent readings of Theorem 10.3 update with every step.
```

**What to notice.** The exact reading $100(1.10^{b_1} - 1)$ and the shortcut $10\,b_1$ never
part company by more than a tenth of a point anywhere on the slider, which is exactly what
the derivation promises for a small coefficient. **Try this.** Push $b_1$ down from $1$ until SSE
stops falling, land near $0.75$, and read off the percent that @ch10-log-interpretation
asks you to report.
...
The profile likelihood is easier to trust once you have watched it fight the residual plot
for a while, so take the dial yourself.

```{iframe} ../sims/ch10-box-cox.html
:class: sim sim-m
:width: 100%
Move $\lambda$ through the Box-Cox family and the 50 cars are refitted from scratch, so the residual funnel of @fig-ch10-cars-funnel reshapes live while $\ell_p(\lambda)$, $\mathrm{RSS}(\lambda)$, and $R^2$ track it.
```

**What to notice.** The profile log-likelihood and the shape of the residual plot agree:
$\ell_p$ climbs from $-135.63$ at $\lambda = 1$ to its peak $-126.73$ at $\lambda = 0.43$
exactly as the funnel flattens, and $\mathrm{RSS}(\lambda)$ alone would have told you
nothing, since it falls all the way to $9.56$ at the log. **Try this.** Find the peak by
hand, then keep going down past $\lambda = 0.2$ and watch a new low outlier open up, the
picture of over-transforming that @ch10-box-cox warns about.
...
Weights are easiest to believe when you can turn them up gradually and watch the line
answer, so slide between ordinary and weighted least squares yourself.

```{iframe} ../sims/ch10-wls.html
:class: sim sim-m
:width: 100%
The slider sets how hard the known precisions push, from $p = 0$ where every strongx measurement votes equally to $p = 1$ where the weight is the textbook $w_i = 1/\text{sd}_i^2$.
```

**What to notice.** At $p = 0$ the weighted line sits exactly on the ordinary one, which is
the derivation's statement that equal weights give back plain least squares. **Try this.**
Raise $p$ to $1$ and watch the slope fall from $619.7$ to $530.8$ while $\mathrm{SE}(b_0)$
drops from $10.08$ to $8.08$: the numbers of Example 10.3, arriving as a rotation you can
see. Keep going to $p = 2$ and ask yourself whether you would still trust a fit resting on
so few points (@ch10-wls).
