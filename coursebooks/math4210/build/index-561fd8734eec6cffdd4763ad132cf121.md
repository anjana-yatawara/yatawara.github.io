...
:class: sim sim-m
:width: 100%
The 25 Toluca runs, with the least-squares line refitting as you drag any point. The readouts show the slope $b_1$, the intercept $b_0$, the predicted hours $\hat{Y}$ at a lot of 100 units and at a lot of 200, and the SSE.
```

**What to notice.** The slope is the description and $\hat{Y}$ is the prediction, and a single
dragged run moves both at once. **Try this.** Drag the run at lot size $120$ straight up and compare
how far $\hat{Y}$ at $100$ moves against $\hat{Y}$ at $200$, a lot size nobody ever ran: the
prediction inside the data barely flinches while the one past the edge swings hard.
Back to [Section 1.1](#ch01-what).
...
:class: sim sim-m
:width: 100%
Galton's least-squares line against the equal-heights line $y = x$, with a slider for mid-parent height. The readouts show how far the parents sit above average, the predicted child $\hat{Y}$, and how far that child sits above average.
```

**What to notice.** The predicted child always lands between the parents' height and the overall
average, on the tall side and on the short side alike. **Try this.** Slide from $75$ inches down to
$65$ and watch the fraction kept hold at $0.637$ the entire way: the same $b_1$ that shrinks a tall
family's advantage lifts a short family's deficit.
Back to [Section 1.2](#ch01-galton).
...
**What to notice.** The count almost never lands on exactly ten, yet the pile it builds is
perfectly orderly. **Try this.** Run 1000 rounds and watch the mean settle near $10$ while the
spread holds near $2.24$: the noise never shrinks, it just becomes describable, which is why a model
carries an $\varepsilon$ instead of pretending the scatter away.
Back to [Section 1.3](#ch01-anatomy).
